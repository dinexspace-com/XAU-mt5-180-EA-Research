...
#property description "EA-062 Body_Breakout"
...
input ulong InpMagicNumber = 123062;
...
input double InpMinBodyRatio = 0.70;
...
bool GetSignal(const MqlRates &bars[],int &direction)
{
   ...
   if(range<=0.0 || MathAbs(bars[1].close-bars[1].open)/range<InpMinBodyRatio) direction=0;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-062")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-062"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(!MathIsValidNumber(InpMinBodyRatio) || InpMinBodyRatio<=0.0 || InpMinBodyRatio>1.0) return INIT_PARAMETERS_INCORRECT;
