...
#property description "EA-080 Impulse_Candle_Break"
...
input ulong InpMagicNumber = 123080;
...
input int InpATRPeriod=14; input double InpImpulseATRMultiplier=1.5; input double InpImpulseBodyRatio=0.70; input int InpImpulseMaxBars=20;
...
datetime impulse_time=0; double impulse_high=0.0,impulse_low=0.0; int impulse_direction=0;
...
bool GetSignal(const MqlRates &bars[],int &direction)
{
   direction=0;
   double atr=0.0;
   if(!ReadIndicator(2,atr) || atr<=0.0) return false;
   const double pad=(double)InpBreakoutBuffer*point_size;
   if(impulse_direction!=0)
   {
      const int age=iBarShift(_Symbol,InpTimeframe,impulse_time,true);
      if(age<0 || age>InpImpulseMaxBars+1) impulse_direction=0;
      else if(bars[1].time>impulse_time)
      {
         if(impulse_direction>0 && bars[1].close>impulse_high+pad) direction=1;
         if(impulse_direction<0 && bars[1].close<impulse_low-pad) direction=-1;
         if(direction!=0) { impulse_direction=0; return true; }
         if((impulse_direction>0 && bars[1].close<impulse_low) ||
            (impulse_direction<0 && bars[1].close>impulse_high)) impulse_direction=0;
      }
   }
   const double range=bars[1].high-bars[1].low;
   const double body=bars[1].close-bars[1].open;
   if(impulse_direction==0 && range>atr*InpImpulseATRMultiplier &&
      range>0.0 && MathAbs(body)/range>=InpImpulseBodyRatio && body!=0.0)
   {
      impulse_time=bars[1].time;
      impulse_high=bars[1].high;
      impulse_low=bars[1].low;
      impulse_direction=(body>0.0 ? 1 : -1);
   }
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-080")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-080"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpATRPeriod<2 || InpATRPeriod>10000 || !MathIsValidNumber(InpImpulseATRMultiplier) || InpImpulseATRMultiplier<=0.0 || !MathIsValidNumber(InpImpulseBodyRatio) || InpImpulseBodyRatio<=0.0 || InpImpulseBodyRatio>1.0 || InpImpulseMaxBars<1 || InpImpulseMaxBars>10000) return INIT_PARAMETERS_INCORRECT; indicator_handle=iATR(_Symbol,InpTimeframe,InpATRPeriod); if(indicator_handle==INVALID_HANDLE) return INIT_FAILED;
