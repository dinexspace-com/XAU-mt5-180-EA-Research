...
#property description "EA-082 Bollinger_Re-entry"
...
input ulong InpMagicNumber = 123082;
...
{
   direction=0;
   double lower1=0.0,upper1=0.0,lower2=0.0,upper2=0.0;
   if(!ReadValue(indicator_handle,2,1,lower1) || !ReadValue(indicator_handle,1,1,upper1) ||
      !ReadValue(indicator_handle,2,2,lower2) || !ReadValue(indicator_handle,1,2,upper2)) return false;
   if(bars[1].close<=lower1 || bars[1].close>=upper1) return true;
   const bool buy=bars[2].close<lower2 || bars[1].low<lower1;
   const bool sell=bars[2].close>upper2 || bars[1].high>upper1;
   // A candle sweeping both bands is ambiguous: do not choose a side arbitrarily.
   if(buy && !sell) direction=1;
   else if(sell && !buy) direction=-1;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-082")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-082"));
   const uint code=trade.ResultRetcode();
