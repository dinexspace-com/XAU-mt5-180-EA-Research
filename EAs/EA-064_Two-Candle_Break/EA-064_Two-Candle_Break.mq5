...
#property description "EA-064 Two-Candle_Break"
...
input ulong InpMagicNumber = 123064;
...
{
   double upper=0.0,lower=0.0;
   // Exclude BOTH confirming candles when defining the fixed breakout range.
   GetRange(bars,3,upper,lower);
   const double pad=(double)InpBreakoutBuffer*point_size;
   direction=0;
   if(bars[2].close>upper+pad && bars[1].close>upper+pad) direction=1;
   else if(bars[2].close<lower-pad && bars[1].close<lower-pad) direction=-1;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-064")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-064"));
   const uint code=trade.ResultRetcode();
