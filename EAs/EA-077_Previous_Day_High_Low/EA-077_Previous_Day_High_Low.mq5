...
#property description "EA-077 Previous_Day_High_Low"
...
input ulong InpMagicNumber = 123077;
...
{
   direction=0;
   const int current=iBarShift(_Symbol,PERIOD_D1,bars[1].time,false);
   if(current<0) return false;
   MqlRates previous[1];
   if(CopyRates(_Symbol,PERIOD_D1,current+1,1,previous)!=1) return false;
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-077")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-077"));
   const uint code=trade.ResultRetcode();
