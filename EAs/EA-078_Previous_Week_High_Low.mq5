...
#property description "EA-078 Previous_Week_High_Low"
...
input ulong InpMagicNumber = 123078;
...
{
   direction=0;
   const int current=iBarShift(_Symbol,PERIOD_W1,bars[1].time,false);
   if(current<0) return false;
   MqlRates previous[1];
   if(CopyRates(_Symbol,PERIOD_W1,current+1,1,previous)!=1) return false;
   if(previous[0].high<=previous[0].low || previous[0].low<=0.0) return false;
   direction=CrossLevel(bars,previous[0].high,previous[0].low);
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-078")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-078"));
   const uint code=trade.ResultRetcode();
