...
#property description "EA-068 Breakout_RSI_Filter"
...
input ulong InpMagicNumber = 123068;
...
input int InpRSIPeriod = 14; input double InpRSIBuyLevel = 55.0; input double InpRSISellLevel = 45.0;
...
{
   double rsi=0.0;
   if(!ReadIndicator(1,rsi)) return false;
   direction=BreakDirection(bars);
   if(direction>0 && rsi<=InpRSIBuyLevel) direction=0;
   if(direction<0 && rsi>=InpRSISellLevel) direction=0;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-068")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-068"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpRSIPeriod<2 || InpRSIPeriod>10000 || !MathIsValidNumber(InpRSIBuyLevel) || !MathIsValidNumber(InpRSISellLevel) || InpRSISellLevel<=0.0 || InpRSIBuyLevel>=100.0 || InpRSISellLevel>=InpRSIBuyLevel) return INIT_PARAMETERS_INCORRECT; indicator_handle=iRSI(_Symbol,InpTimeframe,InpRSIPeriod,PRICE_CLOSE); if(indicator_handle==INVALID_HANDLE) return INIT_FAILED;
