...
#property description "EA-070 Breakout_Volume_Filter"
...
input ulong InpMagicNumber = 123070;
...
{
   double average=0.0;
   // Prior volumes only: do not include the breakout candle in the average.
   for(int i=2;i<InpVolumeLookback+2;i++) average+=(double)bars[i].tick_volume;
   average/=(double)InpVolumeLookback;
   direction=BreakDirection(bars);
   if(average<=0.0 || (double)bars[1].tick_volume<=average) direction=0;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-070")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-070"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpVolumeLookback<2 || InpVolumeLookback>10000) return INIT_PARAMETERS_INCORRECT;
...
void OnTick()
{
   ...
   const int needed=(int)MathMax(InpBreakoutLookback,InpVolumeLookback)+3;
