...
#property description "EA-067 Breakout_EMA_Filter"
...
input ulong InpMagicNumber = 123067;
...
{
   double ema=0.0;
   if(!ReadIndicator(1,ema)) return false;
   direction=BreakDirection(bars);
   if(direction>0 && bars[1].close<=ema) direction=0;
   if(direction<0 && bars[1].close>=ema) direction=0;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-067")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-067"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpEMAPeriod<2 || InpEMAPeriod>10000) return INIT_PARAMETERS_INCORRECT; indicator_handle=iMA(_Symbol,InpTimeframe,InpEMAPeriod,0,MODE_EMA,PRICE_CLOSE); if(indicator_handle==INVALID_HANDLE) return INIT_FAILED;
