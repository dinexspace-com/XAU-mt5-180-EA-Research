...
#property description "EA-076 Round_Number_Break"
...
input ulong InpMagicNumber = 123076;
...
input double InpRoundNumberStep=10.0;
...
{
   direction=0;
   const double previous=bars[2].close;
   const double pad=(double)InpBreakoutBuffer*point_size;
   // Ceil/floor find the next boundary on either side, including an exact grid touch.
   const double upper=MathCeil((previous-pad)/InpRoundNumberStep-1e-10)*InpRoundNumberStep;
   const double lower=MathFloor((previous+pad)/InpRoundNumberStep+1e-10)*InpRoundNumberStep;
   if(previous<=upper+pad && bars[1].close>upper+pad) direction=1;
   else if(previous>=lower-pad && bars[1].close<lower-pad) direction=-1;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-076")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-076"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(!MathIsValidNumber(InpRoundNumberStep) || InpRoundNumberStep<tick_size) return INIT_PARAMETERS_INCORRECT;
