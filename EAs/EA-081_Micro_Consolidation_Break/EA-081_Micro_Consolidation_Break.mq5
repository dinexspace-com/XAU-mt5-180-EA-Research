...
#property description "EA-081 Micro_Consolidation_Break"
...
input ulong InpMagicNumber = 123081;
...
input int InpConsolidationBars=4; input int InpATRPeriod=14; input double InpSmallCandleATR=0.60; input double InpZoneATR=1.20;
...
bool GetSignal(const MqlRates &bars[],int &direction)
{
   direction=0;
   double atr=0.0;
   // Baseline precedes the consolidation: no signal candle in the ATR comparison.
   if(!ReadIndicator(InpConsolidationBars+2,atr) || atr<=0.0) return false;
   double upper=bars[2].high,lower=bars[2].low;
   for(int i=2;i<InpConsolidationBars+2;i++)
   {
      if(bars[i].high-bars[i].low>InpSmallCandleATR*atr) return true;
      upper=MathMax(upper,bars[i].high); lower=MathMin(lower,bars[i].low);
   }
   if(upper<=lower || upper-lower>InpZoneATR*atr) return true;
   const double pad=(double)InpBreakoutBuffer*point_size;
   if(bars[1].close>upper+pad) direction=1;
   else if(bars[1].close<lower-pad) direction=-1;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-081")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-081"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpConsolidationBars<3 || InpConsolidationBars>6 || InpATRPeriod<2 || InpATRPeriod>10000 || !MathIsValidNumber(InpSmallCandleATR) || InpSmallCandleATR<=0.0 || !MathIsValidNumber(InpZoneATR) || InpZoneATR<=0.0) return INIT_PARAMETERS_INCORRECT; indicator_handle=iATR(_Symbol,InpTimeframe,InpATRPeriod); if(indicator_handle==INVALID_HANDLE) return INIT_FAILED;
...
void OnTick()
{
   ...
   const int needed=InpConsolidationBars+3;
