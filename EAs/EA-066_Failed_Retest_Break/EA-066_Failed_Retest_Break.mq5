...
#property description "EA-066 Failed_Retest_Break"
...
input ulong InpMagicNumber = 123066;
...
input int InpRetestTolerance = 20; input int InpRetestMaxBars = 10;
...
int setup_direction=0; datetime setup_time=0; datetime retest_time=0; double setup_level=0.0; double setup_extreme=0.0;
...
{
   direction=0;
   const double tolerance=(double)InpRetestTolerance*point_size;
   const double pad=(double)InpBreakoutBuffer*point_size;
   if(setup_direction!=0)
   {
      const int age=iBarShift(_Symbol,InpTimeframe,setup_time,true);
      if(age<0 || age>InpRetestMaxBars+1) ResetSetup();
      else if(bars[1].time>setup_time)
      {
         // A close back INSIDE the old range invalidates the setup.
         const bool invalid=(setup_direction>0 ? bars[1].close<=setup_level : bars[1].close>=setup_level);
         if(invalid) ResetSetup();
         else
         {
                        // Require a distinct candle AFTER the rejected retest.
            if(retest_time>0 && bars[1].time>retest_time)
            {
               const bool resumed=(setup_direction>0 ? bars[1].close>setup_extreme+pad
                                                      : bars[1].close<setup_extreme-pad);
               if(resumed)
               {
            ...
            const bool touched=(setup_direction>0 ? bars[1].low<=setup_level+tolerance
                                                   : bars[1].high>=setup_level-tolerance);
            const bool bounced=(setup_direction>0 ? bars[1].close>setup_level+pad && bars[1].close>bars[1].open
                                                   : bars[1].close<setup_level-pad && bars[1].close<bars[1].open);
            if(touched && bounced)
            {
               if(retest_time==0) retest_time=bars[1].time;
            }
            return true;
         }
      }
   ...
   if(setup_direction==0)
   {
      const int breakout=BreakDirection(bars);
      if(breakout!=0)
      {
         double upper=0.0,lower=0.0;
         GetRange(bars,2,upper,lower);
         setup_direction=breakout;
         setup_time=bars[1].time;
         setup_level=(breakout>0 ? upper : lower);
         setup_extreme=(breakout>0 ? bars[1].high : bars[1].low);
         retest_time=0;
      }
   }
   return true;
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-066")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-066"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpRetestTolerance<0 || InpRetestMaxBars<1 || InpRetestMaxBars>10000) return INIT_PARAMETERS_INCORRECT; ResetSetup();
