...
#property description "EA-065 Retest_Breakout"
...
input ulong InpMagicNumber = 123065;
   ...
   if(setup_direction!=0)
   {
      ...
      else if(bars[1].time>setup_time)
      {
         ...
         else
         {
            ...
            if(touched && bounced)
            {
               direction=setup_direction; ResetSetup(); return true;
            }
            return true;
         }
      }
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-065")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-065"));
   const uint code=trade.ResultRetcode();
