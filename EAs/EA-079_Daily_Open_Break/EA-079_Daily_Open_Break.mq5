...
#property description "EA-079 Daily_Open_Break"
...
input ulong InpMagicNumber = 123079;
...
{
   direction=0;
   const int shift=iBarShift(_Symbol,PERIOD_D1,bars[1].time,false);
   if(shift<0) return false;
   MqlRates day[1];
   if(CopyRates(_Symbol,PERIOD_D1,shift,1,day)!=1 || day[0].open<=0.0) return false;
   // Do not use yesterday's close as a crossing of today's not-yet-known open.
   if(bars[2].time<day[0].time) return true;
   direction=CrossLevel(bars,day[0].open,day[0].open);
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-079")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-079"));
   const uint code=trade.ResultRetcode();
