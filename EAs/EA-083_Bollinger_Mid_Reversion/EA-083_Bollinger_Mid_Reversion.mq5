...
#property description "EA-083 Bollinger_Mid_Reversion"
...
input ulong InpMagicNumber = 123083;
...
input int InpBandsPeriod=20; input double InpBandsDeviation=2.0;
...
{
   double values[1];
   if(handle==INVALID_HANDLE || BarsCalculated(handle)<=shift || CopyBuffer(handle,buffer,shift,1,values)!=1) return false;
   value=values[0];
   return MathIsValidNumber(value) && value!=EMPTY_VALUE;
}
void ManageMiddleExit()
{
   double middle=0.0;
   if(!ReadValue(indicator_handle,0,1,middle)) return;
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol,tick)) return;
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      const ulong ticket=PositionGetTicket(i);
      if(ticket==0 || PositionGetString(POSITION_SYMBOL)!=_Symbol || (ulong)PositionGetInteger(POSITION_MAGIC)!=InpMagicNumber) continue;
      const bool buy=(ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY;
      if((buy && tick.bid>=middle) || (!buy && tick.ask<=middle))
      {
         ResetLastError();
         const bool ok=trade.PositionClose(ticket);
         const uint code=trade.ResultRetcode();
         if(!ok || (code!=TRADE_RETCODE_DONE && code!=TRADE_RETCODE_DONE_PARTIAL)) LogTradeResult("Middle exit",ok);
      }
   }
}
...
bool GetSignal(const MqlRates &bars[],int &direction)
{
   ...
   if(!ReadValue(indicator_handle,0,1,middle) || !ReadValue(indicator_handle,1,1,upper) || !ReadValue(indicator_handle,2,1,lower)) return false;
   if(bars[1].low<=lower && bars[1].close>lower && bars[1].close<middle && bars[1].close>bars[1].open) direction=1;
   else if(bars[1].high>=upper && bars[1].close<upper && bars[1].close>middle && bars[1].close<bars[1].open) direction=-1;
   // Do not enter after a gap has already carried the tradable price to the target.
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol,tick)) return false;
   if(direction>0 && tick.ask>=middle) direction=0;
   if(direction<0 && tick.bid<=middle) direction=0;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-083")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-083"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   if(!MathIsValidNumber(InpLotSize) || InpLotSize<=0.0 || InpStopLoss<=0 || InpTakeProfit<=0 ||
      InpMagicNumber==0 || InpSlippage<0 || InpMaxSpread<0 ||
      InpBreakoutBuffer<0 ||
   ...
   if(InpBandsPeriod<2 || InpBandsPeriod>10000 || !MathIsValidNumber(InpBandsDeviation) || InpBandsDeviation<=0.0) return INIT_PARAMETERS_INCORRECT; indicator_handle=iBands(_Symbol,InpTimeframe,InpBandsPeriod,0,InpBandsDeviation,PRICE_CLOSE); if(indicator_handle==INVALID_HANDLE) return INIT_FAILED;
