...
#property description "EA-075 Session_Transition_Break"
...
input ulong InpMagicNumber = 123075;
...
input int InpRangeStartHour=0; input int InpRangeEndHour=8; input int InpTradeEndHour=10;
...
// Range is built from completed M1 candles, never from future session bars.
// Defaults are illustrative SERVER hours, not automatic local-session conversions.
bool GetSignal(const MqlRates &bars[],int &direction)
{
   direction=0;
   MqlDateTime parts;
   if(!TimeToStruct(bars[1].time,parts)) return false;
   parts.hour=0; parts.min=0; parts.sec=0;
   const datetime midnight=StructToTime(parts);
   const datetime start=midnight+InpRangeStartHour*3600;
   const datetime finish=midnight+InpRangeEndHour*3600;
   const datetime cutoff=midnight+InpTradeEndHour*3600;
   if(bars[1].time<finish || bars[0].time>=cutoff) return true;
   MqlRates session[];
   const int count=CopyRates(_Symbol,PERIOD_M1,start,finish-1,session);
   if(count<=0 || SeriesInfoInteger(_Symbol,PERIOD_M1,SERIES_SYNCHRONIZED)==0) return false;
   // Require coverage of the entire configured range; reject partially loaded data.
   if(session[0].time!=start || session[count-1].time!=finish-60) return false;
   double upper=session[0].high,lower=session[0].low;
   for(int i=1;i<count;i++)
   {
      upper=MathMax(upper,session[i].high);
      lower=MathMin(lower,session[i].low);
   }
   if(upper<=lower) return true;
   direction=CrossLevel(bars,upper,lower);
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-075")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-075"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(InpRangeStartHour<0 || InpRangeEndHour<=InpRangeStartHour || InpRangeEndHour>=24 || InpTradeEndHour<=InpRangeEndHour || InpTradeEndHour>24 || InpTimeframe!=PERIOD_M1) return INIT_PARAMETERS_INCORRECT;
