...
#property description "EA-063 Close_Location_Break"
...
input ulong InpMagicNumber = 123063;
...
input double InpCloseEdgeFraction = 0.20;
...
bool GetSignal(const MqlRates &bars[],int &direction)
{
   ...
   if(range<=0.0) { direction=0; return true; }
   if(direction>0 && (bars[1].high-bars[1].close)/range>InpCloseEdgeFraction) direction=0;
   if(direction<0 && (bars[1].close-bars[1].low)/range>InpCloseEdgeFraction) direction=0;
   return true;
}
...
void OpenTrade(const int direction)
{
   ...
   const bool ok=(buy ? trade.Buy(InpLotSize,_Symbol,price,sl,tp,"EA-063")
                     : trade.Sell(InpLotSize,_Symbol,price,sl,tp,"EA-063"));
   const uint code=trade.ResultRetcode();
...
int OnInit()
{
   ...
   if(!MathIsValidNumber(InpCloseEdgeFraction) || InpCloseEdgeFraction<=0.0 || InpCloseEdgeFraction>=0.5) return INIT_PARAMETERS_INCORRECT;
